...
export class ChartService {
	...
	constructor() {
		this.chart = new Chart(); // here CIQ.UI.Chart could have been created directly
	}
	...
	createChartAndUI({ container, config }) {
		...
		// attach channel methods to remove the need to provide stx parameter taking 
		this.channelSubscribe = channelSubscribe;

		// Translate breakpoint channel in RxJs stream
		this.channelSubscribe(channels.breakpoint, value => this.breakpoint$.next(value));
